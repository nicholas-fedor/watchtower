...
var _ = ginkgo.Describe("the helpers", func() {
	ginkgo.Describe("GetRegistryAddress", func() {
		...
		ginkgo.It("should return index.docker.io for image refs with no explicit registry", func() {
			...
			gomega.Expect(GetRegistryAddress("nickfedor/watchtower")).
				To(gomega.Equal("index.docker.io"))
		})
		ginkgo.It("should return index.docker.io for image refs with docker.io domain", func() {
			gomega.Expect(GetRegistryAddress("docker.io/watchtower")).
				To(gomega.Equal("index.docker.io"))
			gomega.Expect(GetRegistryAddress("docker.io/nickfedor/watchtower")).
				To(gomega.Equal("index.docker.io"))
		})
		...
		ginkgo.It(
			...
			func() {
				gomega.Expect(GetRegistryAddress("github.com/nicholas-fedor/config")).
