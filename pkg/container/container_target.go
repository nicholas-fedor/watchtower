...
func StartTargetContainer(api client.APIClient, sourceContainer types.Container, networkConfig *network.NetworkingConfig, reviveStopped bool) (types.ContainerID, error) {
	...
	if err := api.ContainerStart(ctx, createdContainer.ID, container.StartOptions{
		CheckpointID:  "",
		CheckpointDir: "",
	}); err != nil {
...
func RenameTargetContainer(api client.APIClient, target types.Container, newName string) error {
	ctx := context.Background()
