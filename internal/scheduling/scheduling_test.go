...
func TestShouldExitDueToInvalidRestart(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "old-named container with run-once false",
			container:    createTestContainer("", withName("watchtower-old-abc123")),
			runOnceFlag:  false,
			expectedExit: true,
		},
		{
			name:         "old-named container with run-once true",
			container:    createTestContainer("", withName("watchtower-old-abc123")),
			runOnceFlag:  true,
			expectedExit: false,
		},
		{
			name:         "old-named container with leading slash",
			container:    createTestContainer("", withName("/watchtower-old-def456")),
			runOnceFlag:  false,
			expectedExit: true,
		},
		{
			name:         "non-old-named container continues",
